...
import { ApiError, useAdminLogin } from "@workspace/api-client-react";
        ...
        onError: (error) => {
          const message =
            error instanceof ApiError && error.data && typeof error.data === "object"
              ...
              : "";
          setErrorMsg(message || "Invalid admin credentials.");
        },
      },
    );
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="w-[800px] h-[800px] rounded-full bg-primary/5 blur-3xl opacity-40 absolute -top-40 -left-40" />
      </div>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-muted-foreground">
          Sign in with your pre-created admin account.
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-card py-8 px-4 shadow-2xl shadow-black/5 border border-border sm:rounded-3xl sm:px-10">
          <form className="space-y-6" onSubmit={handleSubmit}>
            {errorMsg && (
              <div className="rounded-xl bg-destructive/10 p-4 text-sm font-medium text-destructive border border-destructive/20">
                {errorMsg}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isPending}
              className="group w-full flex justify-center items-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 disabled:opacity-70"
            >
              {isPending ? (
                <>
